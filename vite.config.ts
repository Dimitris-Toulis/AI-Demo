					...
					headers.forEach((header) =>
						res.setHeader(
							...
							header.key == "Content-Security-Policy"
								? "default-src 'none'; connect-src 'self' https://storage.googleapis.com https://tfhub.dev; script-src 'self' 'unsafe-eval' blob:; style-src 'self' 'unsafe-inline'; img-src 'self'; object-src 'none'; prefetch-src 'self'; frame-ancestors 'none'; form-action 'none'; block-all-mixed-content; base-uri 'none'; manifest-src 'self'"
								: header.value
						)
					);
					_.url;
